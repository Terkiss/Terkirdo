...
5. **동적 라우팅 (Skill-Aware Decomposition)**
   정적 프롬프트에 의존하지 않고, SKILLWEAVER 논문의 SAD 아키텍처를 도입했습니다. 쿼리를 받으면 백그라운드 FAISS 인덱서를 통해 가용한 스킬 힌트를 먼저 검색한 뒤, 그 실제 어휘(Vocabulary)에 맞춰 하위 작업을 재분해하여 정확도와 토큰 효율을 극대화합니다.
...
| 훅 | 역할 |
|----|------|
| `pre_tool_use_policy.py` | 파괴적 명령어(`rm -rf` 등) 차단 및 워커 실행 시 SAD 타겟 스킬 1개만 프롬프트에 동적 주입하여 토큰 낭비 방지 |
...
| 날짜 | 브랜치 | 내용 |
|------|--------|------|
| 2026-07-08 | `20260708d` | SKILLWEAVER SAD 라우팅 아키텍처 적용, FAISS 스킬 검색기 구축 및 Hook 최적화 |
