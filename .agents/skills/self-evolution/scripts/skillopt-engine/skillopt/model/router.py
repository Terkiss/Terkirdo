...
from . import azure_openai, claude_backend, codex_backend, agy_backend
...
def _all_backend_modules() -> list[Any]:
    return [azure_openai, codex_backend, claude_backend, agy_backend]
...
def set_backend(name: str | None) -> str:
    ...
    if normalized not in {"azure_openai", "codex", "claude", "agy"}:
        valid = ", ".join(sorted({"azure_openai", "codex", "claude", "agy"}))
